...
func main() {
	...
	sort.Sort(gocyclo.ByComplexity(stats))
	written := writeStats(os.Stdout, stats)
	if *avg {
		showAverage(os.Stdout, stats)
	}
	if *over > 0 && written > 0 {
		os.Exit(1)
	}
...
func showAverage(w io.Writer, stats []gocyclo.Stat) {
	fmt.Fprintln(w, fmt.Sprintf("Average: %.3g\n", gocyclo.Average(stats)))
}
